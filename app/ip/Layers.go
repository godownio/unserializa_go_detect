...
// LoadLayers 创建并返回一个包含层信息的fyne容器。
// 返回值 *fyne2.Container: 一个配置有层数据的滚动容器，用于显示层信息。
func LoadLayers() *fyne2.Container {
	...
	// 将滚动容器放置在带边框的容器中，添加分隔符
	PkgInfoContainer := container.NewBorder(nil, speparator, nil, nil, scrollC)
	// 调整容器的大小
	PkgInfoContainer.Resize(fyne2.NewSize(1400, 240))
	return PkgInfoContainer
}

func NewLayersData(FrameNo int, packet gopacket.Packet) map[string][]string {
	// NewLayersData 生成关于数据包各层信息的映射。
	// 参数:
	//   FrameNo int: 数据包的帧编号。
	//   packet gopacket.Packet: 待分析的数据包。
	// 返回值:
	//   map[string][]string: 包含数据包各层信息的映射，其中键是层的类型或描述，值是与该层相关的信息数组。
	// 清空LayersData映射，为新的数据包分析做准备。
