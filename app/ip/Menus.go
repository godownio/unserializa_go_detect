...
func LoadMenus(w fyne.Window, app fyne.App) {
	...
	tools_key := []string{"文件(F)", "过滤(E)", "排序(V)", "开始/切换(W)", "模式(M)", "发送(S)", "捕获(C)"}
	//"捕获(C)", "分析(A)", "统计(S)", "电话(Y)",
	//"无线(W)", "工具(T)", "帮助(H)",
	var tools = [][]string{{"保存", "打开"}, {"过滤器设置"}, {"源IP(递增)", "源IP(递减)", "目的IP(递增)", "目的IP(递减)", "源端口(递增)", "源端口(递减)", "目的端口(递增)",
		"目的端口(递减)", "长度(递增)", "长度(递减)"},
		{"全流量模式", "HTTP流量分析模式"}, {"严格模式", "混杂模式"}, {"发送数据包"}, {"暂停/继续"}}
	var toolsFunc = [][]func(){{
		func() {
				...
				radio := widget.NewRadioGroup(info, func(value string) {
					...
					for _, v := range valid {
						if id == v {
							...
							Ctx, Cancel = context.WithCancel(context.Background())
							time.Sleep(1000 * time.Millisecond)
							go GetHTTPPkg(Ctx, IfaceInfoList[id].NPFName)
							IsGetPkg = true
							IfaceName.Set("当前选择的网卡为: " + value[2:])
							break
						}
...
// 检查并设置容器
// 设置窗口内容为 AllContainer
func SetAllContainer(w fyne2.Window, A fyne2.App) {
...
// 设置窗口内容为指定容器
func SetContainer(w fyne2.Window, A fyne2.App) {
	w.SetContent(NewbackgroundContainer)
	// 重新加载菜单
	//LoadMenus(w, A)
}
